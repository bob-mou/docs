...
1. 测试目的:发现软件中的缺陷
2. 测试依据：需求/产品规格说明书
3. 每一个测试必须在真实产品或虚拟环境运行
4. 测试需要以系统方法展示产品功能：测似点、测试用例、执行记录证明、测试结果、用例执行证明。提交BUG清单让开发进行修改，最后形成测试报告。

### 2、游戏本身的测试

#### 	a、游戏测试的特性

1. 游戏情节的测试：游戏任务系统的组成，即游戏的时间驱动
2. 游戏平衡的测试：经济平衡，能力平衡（包括技能、属性、种族、装备、经济系统等），保证公平性
3. 游戏文化的测试：游戏风格是中国主导还是日韩风主导，从NPC到游戏整体。
...
目的：确保游戏在弱网、丢包、高延迟、无网络的情况下也能正常运行，验证弱网状态的下的提示是否友好，验证弱网环境下的短线重连机制的数据同步、正确和唯一性，测试避免造成用户流失
...
2. [模拟面试（不完整）](https://bob-mou.github.io/docs/Document/模拟面试（不完整）.m4a)                                                        [模拟面试（不完整）本地](
